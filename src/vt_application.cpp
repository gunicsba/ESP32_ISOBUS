...
void SeederVtApplication::update_pca()
{
	for(int i = 0 ; i < sectionControl.get_number_of_sections() ; i++){
		sectionControl.get_section_setpoint_state(i);
	}
}
...
void SeederVtApplication::update_speedometer_objects(std::uint32_t speed)
{
	...
	switch (VTClientInterface->languageCommandInterface.get_commanded_distance_units())
	{
		...
		{
			// Scale to KPH
			speed = static_cast<std::uint32_t>(((double)speed * 0.001f) * 3.6f); // Converting mm/s to m/s, then mm/s to kph
		}
		break;
		...
		{
			// Scale to MPH
			speed = static_cast<std::uint32_t>(((double)speed * 0.001f) * 2.23694f); // Converting mm/s to m/s, then mm/s to mph
		}
		break;
